...
class Network(object):
  ...
  def build_output_ops(self, layer, network_output_type, output_size, 
    ...
    if network_output_type == 'normal':
      ...
      for idx, hidden_size in enumerate(output_hidden_sizes):
        ...
        layer, self.var[w_name], self.var[w_name] = \
            linear(layer, hidden_size, weights_initializer,
              biases_initializer, output_hidden_activation_fn, trainable, name='out_lin_%d' % idx)

      idx += 1
      w_name, b_name = 'out_w_%d' % idx, 'out_b_%d' % idx

      self.outputs, self.var[w_name], self.var[w_name] = \
          linear(layer, output_size, weights_initializer,
            biases_initializer, output_activation_fn, trainable, name='out_lin_%d' % idx)
      ...
      for idx, hidden_size in enumerate(value_hidden_sizes):
        ...
        layer, self.var[w_name], self.var[b_name] = \
            linear(layer, hidden_size, weights_initializer,
              biases_initializer, output_hidden_activation_fn, trainable, name='val_lin_%d' % idx)
      ...
      for idx, hidden_size in enumerate(advantage_hidden_sizes):
        ...
        layer, self.var[w_name], self.var[b_name] = \
            linear(layer, hidden_size, weights_initializer,
              biases_initializer, output_hidden_activation_fn, trainable, name='adv_lin_%d' % idx)
